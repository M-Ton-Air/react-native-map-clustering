...
import { AqicnDataEntity } from '../../../src/models/index';
import {
  isMarker,
...
const ClusteredMapView = forwardRef(
  (
    ...
    /**
     * M' Ton Air project.
     * returns the cluster color if getText is set to false, the cluster text otherwise.
     * @param {*} cluster 
     * @param {Boolean} getText 
     */
    const _getClusterProperties = (cluster, getText) =>
    {
      ...
      {
        let title = markers[i].properties.title;
        if(title)
        {
          let aqi = parseInt((title.substring(title.indexOf('AQI :'), title.length)).replace('AQI : ', ''));
          if(aqi !== 'undefined')
          {
            clusterAqi += aqi;
          }
      ...
      let avg = clusterAqi/markers.length;
      if(getText)
      {
        return Math.round(avg);
      }

      return AqicnDataEntity.aqiToHexadecimalColor(avg);
    };
          ...
          marker.properties.point_count === 0 ? (
            ...
            renderCluster ? (
              ...
              <ClusterMarker
                ...
                identifier={`cluster-${marker.id}`}
                {...marker}
                onPress={_onClusterPress(marker)}
                clusterColor={_getClusterProperties(marker, false)}
                clusterTextColor={clusterTextColor}
                clusterFontFamily={clusterFontFamily}
                tracksViewChanges={tracksViewChanges}
                useCustomClusterText={true}
                customClusterText={_getClusterProperties(marker, true)}
              />
            )
          ) : null
        )}
